...
/**
 * Menangani perubahan status login
 * Ini adalah "gerbang utama" aplikasi
 */
function handleAuthState(user) {
    if (user) {
        ...
        // Update profil di Firestore (tidak harus ditunggu)
        updateUserProfile(user);
        
        // Sinkronkan data local (jika ada) ke cloud
...
/**
 * Wrapper untuk memuat data.
 * Otomatis memilih local atau cloud.
 */
function loadData() {
    if (state.isLoggedIn) {
...
/**
 * Memuat data dari localStorage (untuk guest)
 */
function loadDataFromLocalStorage() {
    try {
        ...
        if (savedData) {
            ...
            if (today.toDateString() === lastSaveDate.toDateString()) {
...
/**
 * Menyimpan data ke localStorage (untuk guest)
 */
function saveDataToLocalStorage() {
    if (state.isLoggedIn) return; // Jangan simpan local jika login
    try {
        const dataToSave = {
...
function startActivity() {
    if (state.currentActivity) {
        stopActivity(); // Hentikan yang lama dulu
    }
...
function stopActivity() {
    ...
    const duration = Math.round((endTime - state.startTime) / 1000); // dalam detik

    state.accumulated[state.currentActivity] += duration;
    
    // Siapkan data aktivitas untuk disimpan
    const activityData = {
    ...
    // Otomatis simpan ke cloud atau local
    if (state.isLoggedIn) {
...
function init() {
    ...
    // Ini akan memicu `handleAuthState` yang akan memuat data
